  ...
  function createScenes(){
    ...
    /*daruk = createCollada("daruk");
    link = createCollada("link");
    zelda = createCollada("zelda");*/
  ...
  /*function createCollada(name){
    var object;
    var loadMan = new THREE.LoadingManager( function(){
      scene4.add( object );
    });
    var nombre = name.concat("/", name);
    var pre = "./models/";
    var pos = ".dae";
    var file = pre.concat(nombre, pos);
    var loader = new THREE.ColladaLoader( loadMan );
  ...
  function updateScenes(markers){
    ...
    if (markers.length > 0){
      ...
      if(markers[0].id < 100){
        ...
        //updateObject(link, pose.bestRotation, pose.bestTranslation);
        playAudio(1);
      }
      //van halen
      else if(markers[0].id < 220){
        updateObject(model2, pose.bestRotation, pose.bestTranslation);
        //updateObject(daruk, pose.bestRotation, pose.bestTranslation);
        playAudio(2);
      }
      //u2
      else if(markers[0].id < 340){
        updateObject(model3, pose.bestRotation, pose.bestTranslation);
        //updateObject(zelda, pose.bestRotation, pose.bestTranslation);
        playAudio(3);
      }
      //scorpions
...
/*      updatePose("pose1", pose.bestError, pose.bestRotation, pose.bestTranslation);
      ...
      model2.rotation.z -= step;
      model3.rotation.z -= step;
      model4.rotation.z -= step;
      model5.rotation.z -= step;
      model6.rotation.z -= step;
    }
    else{
      pauseAudio();
      hideObject(model1);
